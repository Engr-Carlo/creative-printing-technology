...
import { addItemNote } from "@/app/actions/notes";
...
export default function ItemNoteCell({ itemId, notes }: ItemNoteCellProps) {
  ...
  const handleSubmit = () => {
    ...
    startTransition(async () => {
      const result = await addItemNote(itemId, content);
  ...
  return (
    <div className="relative">
      ...
      {open && (
        <div className="absolute right-0 top-full mt-1 z-50 w-64 bg-white border border-gray-200 rounded-lg shadow-lg p-2 space-y-1.5">
          {/* Add note input */}
