      ...
      <div className={`px-4 pt-4 pb-3 flex items-start justify-between gap-2 ${tColor.bg}`}>
        <div className="flex items-center gap-2.5 min-w-0">
          ...
          <div className="min-w-0">
            ...
            {machine.handlesProcesses.length > 0 && (
              <div className="flex flex-wrap gap-1 mt-1">
                {machine.handlesProcesses.map((p) => (
                  <span key={p} className={`inline-flex items-center gap-0.5 text-[9px] font-bold px-1.5 py-0.5 rounded-md border ${tColor.text} ${tColor.bg} ${tColor.border}`}>
      ...
      <div className="flex-1 px-4 py-3 space-y-3">
        ...
        {status === "BUSY" && machine.currentJob ? (
          ...
          <div className="space-y-1.5">
            ...
            {/* Fleet active — shows floor-level load for this machine's process type */}
            {machine.fleetActiveEstimate > 0 && (
              <div className="rounded-xl bg-blue-50 border border-blue-100 px-3 py-2 flex items-center gap-2">
                ...
                <span className="text-[10px] text-blue-700 font-semibold">
                  ~{machine.fleetActiveEstimate} active job{machine.fleetActiveEstimate !== 1 ? "s" : ""} on floor
                </span>
              </div>
            )}
          </div>
